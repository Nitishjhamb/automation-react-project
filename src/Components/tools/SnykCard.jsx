...
import vulnerabilitiesData from "../../../client/public/snyk-report.json";

const SnykCard = () => {
  const totalDependencies = vulnerabilitiesData.dependencyCount;
  const totalVulnerabilities = vulnerabilitiesData.uniqueCount;
  const vulnerabilities = vulnerabilitiesData.vulnerabilities || [];

  const severityCounts = vulnerabilities.reduce((acc, vuln) => {
    const severity = (vuln.severityWithCritical || vuln.severity).toLowerCase();
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    <span
                      className={`px-2 py-1 text-xs rounded-full whitespace-nowrap ${getSeverityColor(
                        vuln.severityWithCritical || vuln.severity
                      )}`}
                    >
                      {formatSeverityText(
                        vuln.severityWithCritical || vuln.severity || "unknown"
                      )}
                    </span>
                    {vuln.references && vuln.references[0] && (
                      <a
